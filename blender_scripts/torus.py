...
cam_obj = setup_camera(
    offset=(0, 0, -0.25),
    distance=24.75,
    angle=(-5 * np.pi / 36, 0, 0),
    lens=85,
    height=430,
    crop=(1 / 12, 1, 1 / 12, 5 / 6),
)
setup_lighting(
    shifts=(-15, -15, 15),
    sizes=(15, 24, 9),
    energies=(3000, 625, 1000),
    horizontal_angles=(-np.pi / 4, np.pi / 3, np.pi / 4),
    vertical_angles=(-np.pi / 3, -np.pi / 4, np.pi / 4),
)
set_resolution(640)
...
vf_bm = import_vector_field(os.path.join(data_dir, "torus", f"sample_vecs.csv"))
vf_obj = add_vector_field(vf_bm, arr_obj, scale=1.5, name="sample")

set_object_collections(object=[obj, vf_obj])

bpy.ops.object.empty_add(
    type="PLAIN_AXES", align="WORLD", location=obj.location, scale=(1, 1, 1)
)
