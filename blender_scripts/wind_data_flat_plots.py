...
setup_layers()
setup_compositor(
    mask_center=(0.5, 0.5), mask_size=(1.5, 1.0), shadow_color_correction_exponent=2.75
)
...
set_resolution(430, aspect=(2, 1), crop=(0, 1, 0, 1))
...
line_obj = import_curve(
    os.path.join(data_dir, render_name, "earth_flat_line.csv"), name="Line"
)
add_line_color(line_obj, (1, 0, 0, 1))
line_obj2 = import_curve(
    os.path.join(data_dir, render_name, "earth_flat_line2.csv"), name="Line"
)
...
vf_bm = import_vector_field(os.path.join(data_dir, render_name, f"tracks_flat.csv"))
...
mean_bm = import_vector_field(os.path.join(data_dir, render_name, f"mean_s2_flat.csv"))
...
mean_obj.modifiers["means"].node_group.nodes[
    "Attribute Combine XYZ"
].input_type_x = "FLOAT"
mean_obj.modifiers["means"].node_group.nodes["Attribute Combine XYZ"].inputs[
    2
].default_value = width
mean_obj.modifiers["means"].node_group.nodes[
    "Attribute Combine XYZ"
...
cc_bm = import_vector_field(
    os.path.join(data_dir, render_name, f"covariance_r2_flat.csv")
)
...
set_object_collections(
    object=[earth_obj, vf_obj, mean_obj, line_obj, line_obj2, cc_obj]
)
...
cleanup(
    objects=[earth_obj, vf_obj, mean_obj, cc_obj], materials=[earth_mat], force=True
)

bm = import_bmesh(os.path.join(data_dir, render_name, "earth_flat.obj"))
import_color(
    bm,
    data_file=os.path.join(data_dir, render_name, "s_r2.csv"),
    palette_file=os.path.join(col_dir, "viridis.csv"),
)
earth_obj = add_mesh(bm, name="Earth")
earth_mat = add_vertex_colors(earth_obj)
