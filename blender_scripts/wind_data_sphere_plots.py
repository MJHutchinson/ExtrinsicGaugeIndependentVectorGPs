...
line_obj = import_curve(
    os.path.join(data_dir, render_name, "earth_line.csv"), name="Line"
)
...
mean_bm = import_vector_field(os.path.join(data_dir, render_name, f"mean_r2.csv"))
...
empty.rotation_euler = Euler(
    (math.radians(160), math.radians(-25), math.radians(125)), "XYZ"
)
empty.scale = (-1, 1, 1)
