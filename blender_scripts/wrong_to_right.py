...
import_color(
    bm,
    name="s_right",
    data_file=os.path.join(data_dir, "kernels", "s_right.csv"),
    palette_file=os.path.join(col_dir, "viridis.csv"),
)
...
# VECTOR FIELD
vf_bm = import_vector_field(os.path.join(data_dir, "unwrap_sphere", f"frame_59.csv"))
...
mean_bm = import_vector_field(os.path.join(data_dir, "unwrap_sphere", f"frame_59.csv"))
...
vec_fraction_node = mix_geometry_attributes(
    mean_obj, ["arrow", "normal_x", "normal_z"], "_wrong", "_right"
)
