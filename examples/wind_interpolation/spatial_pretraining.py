...
def main(logdir, samples, epochs, geometry):
    ...
    log_length_scale, log_amplitudes = [], []
    for i, idx in enumerate(idxs[:samples]):
        ...
        log_length_scale.append(log_length_scale)
        log_amplitudes.append(log_amplitude)

    log_length_scale = np.stack(log_length_scale)
    log_amplitudes = np.stack(log_amplitudes)

    plt.figure()
    plt.hist(log_length_scale)
    ...
    with open(logdir + "/" + geometry + "_params_pretrained.pickle", "wb") as f:
        pickle.dump(
            {"log_length_scale": log_length_scale, "log_amplitude": log_amplitudes}, f
        )
