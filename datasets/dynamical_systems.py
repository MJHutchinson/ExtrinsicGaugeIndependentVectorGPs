...
class DynamicalSystem(ABC):
    ...
    def rollout(
        ...
        def unvectozied_scan(initial_state: jnp.array):
            ...
            (_,output_array) = jax.lax.scan(scan_function, initial_state, None, length=num_steps)
            return output_array
...
class PendulumSystem(HamiltonianSystem):
    ...
    def hamiltonian(
        ...
        return jnp.squeeze(p_theta**2 / (2*self.mass*self.length**2) + self.mass*self.gravity*self.length*(1 - jnp.cos(theta)), axis=-1)
...
class GPDynamicalSystem(DynamicalSystem):
    ...
    def step(
        ...
        deltas = self.gp(gp_params, gp_state, states)
